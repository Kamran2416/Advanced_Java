...
public class PongCourt extends JPanel {
    public static int lives; // Variable that stores how many lives the player has left. The player loses the
                             // game if he/she dies with zero lives remaining.
    public static int score; // Stores the total score of the player. 10 points for each enemy killed, and 10
                             // points for each coin collected.
    public static int level; // Stores the level the player is currently on.
    public int goombaCount; // The number of enemies the player must kill in order to advance to the next
                            // level.
    public static PictureObj bgImage; // Stores the background image.
    public static Color bgColor; // Stores the background color.
    public static HashSet<PowerUp> powerUpsInPlay; // HashSet that stores the powerups that have been released but not
                                                   // collected.
    ...
    public static AnimatedCharacter boss; // If the level has an AnimatedCharacter boss, it is stored here. Otherwise,
    ...
    public static Timer timer2; // I use this timer to help animate the player's running.
                                // Could easily have stored it in the Player class, but I don't think it makes a
                                // difference.
    public static Timer goombaGenTimer; // Timer that is responsible for spawning new enemies and checking for win/lose
                                        // conditions.
    ...
    boolean starting; // Boolean that checks whether the game is just starting up or not. Used to
                      // prevent the game from executing tick()
                      // or repaint() before everything has been initialized.
    public HashSet<BigBlooper> bloopers; // HashSet that stores the Big Bloopers in the level. Used only in level 5. I
                                         // didn't have BigBlooper
                                         // extend Goomba because I wanted a completely different kind of behavior,
                                         // which was neither that of
    ...
    public PongCourt() {
        ...
        addKeyListener(new KeyAdapter() {
            ...
            public void keyTyped(KeyEvent e) {
                // Prevents the player from jumping unless they are not already jumping and on
                // the ground.
                if (e.getKeyChar() == ' ' && (player.onSolidGround) && player.state != CharacterState.JUMPING) {
                    player.onSolidGround = false;
                    if (player.jumpcount % 3 == 0) {
                        new AePlayWave(player.sfx[1]).start();
                    } else if (player.jumpcount % 3 == 1) {
                        new AePlayWave(player.sfx[2]).start();
                    } else if (player.jumpcount % 3 == 2) {
                        new AePlayWave(player.sfx[3]).start();
                    }
                    player.jumpcount++;
            ...
            public void keyReleased(KeyEvent e) {
                ...
                if ((e.getKeyCode() == KeyEvent.VK_LEFT || e.getKeyCode() == KeyEvent.VK_RIGHT)) {
                    ...
                    if (player.state != CharacterState.JUMPING) {
        ...
        musicTimer = new Timer(10, new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                if (backgroundTheme == null || backgroundSource == null) {
                    return;
                }
        ...
        // Initializes and starts the player's powerup timer. This will bring the player
        // back to his normal state 15 seconds after using a powerup.
    ...
    // Method that creates a Player named "Mario" if the player chooses to play as
    // Mario.
    ...
    // Prompts the player for who they would like to play as. Does not let the user
    // proceed until they choose either Mario or Bowser.
    ...
    public void startLevel1() {
        ...
        // Initializes and starts the main enemy generator and win/lose condition
        // checking timer. Has different conditions for different levels.
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (goombas.size() <= goombaCount) {
                    ...
                    while (goombit.hasNext()) {
                        if (goombit.next().state != CharacterState.DEAD) {
                            allDead = false;
                            break;
                        }
                    }
    ...
    // Method that is responsible for directing the player to the next level. If the
    // player beats level 5, he/she wins the game.
    public void checkForLevel() {
        if (backgroundTheme != null) {
            backgroundTheme.nBytesRead = -1;
        }
        timer.stop();
        if (player != null) {
        ...
        bloopers = null;
        if (level - 1 != 0) {
            JOptionPane.showMessageDialog(null, "Level " + (level - 1) + " cleared!");
        }
        goombaCount = 10;
        if (level == 1) {
            startLevel1();
        ...
        JOptionPane.showMessageDialog(null, "Congratulations! You have won the game with a score of " + score
                + ". Press 'R' or click the Reset button to play again.");
        goombaGenTimer.stop();
    ...
    void tick() {
        ...
        // Maps out the movements, collisions, and interactions of each of the objects
        // in the game.
        ...
        while (goombit.hasNext()) {
            ...
            if (goomba.state != CharacterState.DEAD) {
                ...
                if (!player.invincible) {
                    ...
                    if (mn == Intersection.DOWN || (player.hasMushroom && mn != Intersection.NONE)) {
                        goomba.state = CharacterState.DEAD;
                        goomba.setCurrentSprite();
        ...
        if (powerUpsInPlay != null) {
            ...
            while (pups.hasNext()) {
                ...
                while (blks.hasNext()) {
                    ...
                    if (m == Intersection.DOWN) {
                        onTheGround = true;
                        break;
                    }
                }
                if (!(onTheGround || pow.y + pow.height <= COURTHEIGHT)) {
                    pow.velocityX = 0;
                }
        ...
        while (blks.hasNext()) {
            ...
            if (i == Intersection.DOWN) {
                ...
                player.onSolidGround = player.onSolidGround || false;
            }
        }
    ...
    public void paintComponent(Graphics g) {
        ...
        if (bgImage != null) {
            g.drawImage(bgImage.toImage(), 0, 0, 1200, 800, null);

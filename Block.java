...
public class Block extends GameObject {
    ...
    public static void addSteps(int numBlocks, int orientation, int startingX, int startingY) {
        ...
        while (numBlocks > 0) {
            if (Math.random() < 0.75) {
                PongCourt.blocks.add(new Block(startingX + (50 * orientation * i), startingY - (50 * i)));
            } else {
                PongCourt.blocks
                        .add(new ItemBlock(startingX + (50 * orientation * i), startingY - (50 * i), -orientation));
            }
            numBlocks--;
            i++;
        }
    ...
    // Adds a horizontal line of bricks to the HashSet stored in PongCourt given a
    // length and leftmost point.
    public static void addHorizontalLine(int numBlocks, int startingX, int startingY) {
        int i = 0;
        while (numBlocks > 0) {
            if (Math.random() < 0.75) {
                PongCourt.blocks.add(new Block(startingX + 50 * i, startingY));
            } else {
    ...
    // Adds the arrangement of bricks in level 3 to the HashSet stored in the
    // PongCourt.

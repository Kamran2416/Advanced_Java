...
public abstract class GameObject {
    ...
    // Alternative collision scheme that uses coordinate checking to determine the
    ...
    public Intersection collideByCoords(GameObject other) {
        ...
        if (down > other.y && down < odown && x < oright && right > other.x) {
            clip(other, Intersection.DOWN);
            return Intersection.DOWN;
        }
        if (y < odown && y > other.y && x < oright && right > other.x) {
            clip(other, Intersection.UP);
            return Intersection.UP;
        }
        if (right < oright && right > other.x && down > other.y && y < odown) {
            clip(other, Intersection.LEFT);
            return Intersection.LEFT;
        }
        if (x > other.x && x < oright && down > other.y && y < odown) {
            clip(other, Intersection.RIGHT);
    ...
    // Keeps colliding objects from overlapping. Also responsible for handling some
    // collisions (like between player and ItemBlock)
    ...
    // Keep the object in the bounds of the court. Also plays a sound effect and
    // makes new dialog text if the player hits the side.
    public void clip() {
        if (x < 0) {
            while (x < 10) {
                x += 2;
